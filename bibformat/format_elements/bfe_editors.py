...
def format_element(bfo, limit, separator=' and ', extension='[...]', print_links="yes"):
    """
    ...
    #Originally editors only employed 100 field.  It is now expanded to cover multiple editors.    
    moreauthors = bibrecord.record_get_field_instances(bfo.get_record(), '700')
    moreeditors = [bibrecord.field_get_subfield_values(moreauthor, 'a')[0]
               for moreauthor in moreauthors if len(bibrecord.field_get_subfield_values(moreauthor, "e")) > 0 and bibrecord.field_get_subfield_values(moreauthor, "e")[0]=="ed." ]

    editors = editors + moreeditors
