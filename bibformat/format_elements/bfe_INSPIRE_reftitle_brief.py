...
"""BibFormat element - Print titles in the most flexible reasonable fashion
   ...
   The purpose of this BFE is pretty much to just get reference titles for papers in INSPIRE 
   to display as a short-title stub + ... as a hot link for brevity.
...
def format_element(bfo, highlight="no", force_title_case="no", brief="no", esctitle='1', oldtitles="no"):
    ...
    for title in titles:
        ...
        ref_title = title.get('a', '')

#This instruction truncates the title to <= 6 non-space strings + ...

        truncated_title = ' '.join(re.findall(r'\b\w+\b', ref_title)[0:6]) + '...'
        out += truncated_title
