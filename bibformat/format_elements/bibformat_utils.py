...
def record_get_xml(recID, format='xm', decompress=zlib.decompress,
    ...
    if format.startswith("xm") or format == "marcxml":
        ...
        # Sequence to generate special contract XML for US DoE Office of Scientific and Technical Information
        # 29/08/2013 R.A.
        if record_exist_p == -1:
            out += ""
        else:
            for f in get_fieldvalues(recID, "970__a"):
                if "SPIRES" in f:  #we have to remove 'SPIRES-' from the rec.number
                    f2 = re.sub(r'SPIRES-(\d+)\s*$',r'\1',f)
                    out += "      <accession_num>%s</accession_num>\n" % f2
                else:              #just show regular inspire number
                    out += '      <accession_num>' + str(recID) +  '</accession_num>\n'
        #SPIRES record numbers are used if available to prevent OSTI loading duplicate records from INSPIRE
        #that they already got from SPIRES in years past.

            for f in get_fieldvalues(recID, "245__a"):
                out += "    <title>\n       %s\n      </title>\n" % encode_for_xml(f)

        #The authors in the ostixml are all strung togehter between author tags delimited by ;
        #If zero or > 10 authors don't show any authors
            firstauth = get_fieldvalues(recID, "100__a")
            if len(firstauth) > 0:
                for f in firstauth:
                    FA += "      <author>%s;" % encode_for_xml(f)

            addedauth = get_fieldvalues(recID, "700__a")
            if len(addedauth) < 10 and len(addedauth) > 0:
                out += FA
                for f in addedauth:
                    out += " %s;" % encode_for_xml(f)
                out += "</author>\n"
            elif len(firstauth) == 0 or len(addedauth) > 9:
                out += ''
            else:
                out += FA + "</author>\n"

            for f in get_fieldvalues(recID, "65017a"):
                if ostifoo.has_key(f):  #ostifoo defined at start of def
                    out += "      <subj_category>%s</subj_category>\n" % str(ostifoo[f])
                out += "      <subj_keywords>%s</subj_keywords>\n" % encode_for_xml(f)

            for f in get_fieldvalues(recID, "269__c"):
                out += "      <date>%s</date>\n" % f

        #Fermilab report numbers mapped to OSTI doc types
            for f in get_fieldvalues(recID, "037__a"):
                if "Fermilab" in f or "FERMILAB" in f:
                    if "PUB" in f:
                        doctype = 'JA'
                    elif "CONF" in f:
                        doctype = 'CONF'
                        ...
                        doctype = 'TM'
                    out += "      <doctype>" + doctype + "</doctype>\n"
        #One MARC field is used for conferences and journals.  So, the following journal coding
        #handles the variations, and outputs journal and conf. cites in a nice order.
        #If the conf has a cnum, we get the conf info from its separate record in the conf. "collection."
        #There are a few if-then gymnastics to cover possible missing information and still make a note
        #that looks okay (sort of)
            bfo=BibFormatObject(recID)
            journals = bfo.fields('773__', repeatable_subfields_p=True)
            for journal in journals:
                if journal.has_key('p'):
                    out += "      <journal_info>"
                    journal['p'] = journal['p'][0]
                    title = journal['p']
                    title = encode_for_xml(title)
                    out += str(title) #This can also be "conf.proc." formatted stuff
                if journal.has_key('v'):
                    journal['v'] = journal['v'][0]
                    vol = journal['v']
                    out += ' ' + str(vol)
                if journal.has_key('c'):
                    journal['c'] = journal['c'][0]
                    pages = journal['c']
                    if journal.has_key('p'):
                        out += ':' + str(pages)
                if journal.has_key('y'):
                    journal['y'] = journal['y'][0]
                    year = journal['y']
                    out += ',' + str(year)
                if journal.has_key('p'):
                    out += "</journal_info>\n"
                if journal.has_key('w'):
                    journal['w'] = journal['w'][0]
                    cnum = journal['w']
                    cnum = cnum.replace("/", "-")
                    search_res = search_pattern(p="111__g:" + str(cnum) + " and 980__a:CONFERENCES")
                    idrec = list(search_res)[0] #conf. rec. no. in intbitset
                    conftitle = get_fieldvalues(idrec,'111__a',repetitive_values=True)[0]
                    confplace = get_fieldvalues(idrec,'111__c',repetitive_values=True)[0]
                    confdate = get_fieldvalues(idrec,'111__d',repetitive_values=True)[0]
                if journal.has_key('t'):
                    journal['t'] = journal['t'][0]
                    confinfo = journal['t'] #"presented at:, invited talk:, title maybe etc."
                if journal.has_key('t') or journal.has_key('w'):
                    confstring = '      <conf_info>'  #if we don't have t info or cnum the hell with it
                    if journal.has_key('t'):
                        confstring += str(confinfo) + ':  '
                    if journal.has_key('w'):
                        confstring += str(conftitle) + '.  ' + str(confplace) + ', ' + str(confdate) + '.  '
                    if journal.has_key('c'):
                        confstring += 'pp: ' + str(pages) + '</conf_info>\n'
                    else:
                        confstring += '</conf_info>\n'

            for f in get_fieldvalues(recID, "0247_a"):
                out += "      <doi>%s</doi>\n" % f

            if confstring != '':
                out += encode_for_xml(confstring)  #because it has to come after doi

            for f in get_fieldvalues(recID, "300__a"):
                out += "      <format>%s pages</format>\n" % f

            for f in get_fieldvalues(recID, "041__a"):
                out += "      <language>%s</language>\n" % encode_for_xml(f)
        #As with journals, eprints are in with report nos. in our MARC format so they have to be
        #separated out
            eprnt = False
            for f in get_fieldvalues(recID, "037__a"):
                if "arXiv" in f:
                    eprnt = True  #flag set to trigger later output in url section
                    eprint = f
                    out += "      <arXiv_eprint>arXiv eprint number %s</arXiv_eprint>\n" % f
                else:
                    out += "      <report_number>%s</report_number>\n" % f

            urls = bfo.fields('8564_', repeatable_subfields_p=True)
            for url in urls:
                if url.has_key('y'):
                    url['y'] = url['y'][0]
                    if "FERMILAB" in url['y']:
                        if url.has_key('u'):
                            url['u'] = url['u'][0]
                            lssurl = url['u']
                            out += '      <url>' + str(lssurl) + '.pdf' + '</url>\n'

            if eprnt:
                out += "      <availability>http://arXiv.org/abs/" + eprint + "</availability>\n"

            out += "      <sponsor_org>DOE Office of Science</sponsor_org>\n"

            for f in get_fieldvalues(recID, "961__c"):
                out += "      <dt_harvest>%s</dt_harvest>\n" % f
