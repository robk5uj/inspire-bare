...
def format_element(bfo, reference_prefix, reference_suffix):
    ...
    for reference in references:
        eprint = False
        ref_out = []
        ref_out.append('<tr><td valign="top" align="right">')
...
#n and 0 have recid number
        if reference.has_key('n'):
            linker = reference['n'][0]
            hits = [linker]

        if reference.has_key('0'):
            minker = reference['0'][0]
            hits = [minker]
...
##c_count is citation count
            ...
            ref_out.append(format_record(list(hits)[0],'hs2') + '  <b>[' + "<a href=\"/search?ln=en&amp;p=refersto%3Arecid%3A" + refersto_rec + "\">" + c_count + "</a>"  + ' citations in HEP]</b>')
        else:
#else if the reference is not in inspire:
            if reference.has_key('a'):
                ref_out.append("<a href=\"http://dx.doi.org/" + reference['a'][0] + "\">" + reference['a'][0]+ "</a>")
#h is author
            if reference.has_key('h'):
                ref_out.append(" " + reference['h'][0] + ",")
#t is title
            if reference.has_key('t'):
                ref_out.append(" <i>" + reference['t'][0] + "</i> -")
#p is publisher
            if reference.has_key('p'):
                ref_out.append(" " + reference['p'][0] + ".")
#y is year
            if reference.has_key('y'):
                ref_out.append(" " + reference['y'][0] + ".")
#m is miscellaneous i.e. anything refextract dumps because it doesn't know what else to do
            if reference.has_key('m'):
                ref_out.append(" "+ reference['m'][0].replace(']]', ']') + ".")
#u is various URLs
            if reference.has_key('u'):
                ref_out.append(" <a href=\"" + reference['u'][0] + "\">" + \
                reference['u'][0] + "</a>")
#i is ISBNs
            if reference.has_key('i'):
                for r in reference['i']:
                    ref_out.append(" <a href=\"/search?ln=en&amp;p=020__a%3A"+r+"\">"+r+"</a>")
...
#sequence of if then else to create URLs for cite count, ADS, APS DOIs, and crossref links by priority
            ...
            if display_journal:
                ...
                ref_out.append('  <b>[' + "<a href=\"/search?ln=en&amp;p=fin" + "+" + "c" + "+" + clean_journal + "\">" + c_count + "</a>" + ' citations in HEP]</b>')
                ...
                if ajs.has_key(jn):
                    ads_name = str(ajs[jn])
                    ref_out.append('  <b>[' + "<a href=\"http://adsabs.harvard.edu/cgi-bin/nph-abs_connect?bibstem=" + ads_name + "&amp;volume=" + vol + "&amp;page=" + pp + "\">" + 'ADS' + "</a>" + ']</b>')
                if re.search("^Phys\.Rev\.$", jn) or re.search("^Phys\.Rev\.Lett\.$", jn) or re.search("$Rev\.Mod\.Phys\.$", jn):
                    ...
                    if letnum:
                        l = letnum.group('let')
                        n = letnum.group('num')
                        ref_out.append('  <b>[' + "<a href=\"http://dx.doi.org/10.1103/" + apsjn + l + '.' + n + '.' + pp + "\">" + 'DOI' + "</a>" + ']</b>')
                    else:
                        ref_out.append('  <b>[' + "<a href=\"http://dx.doi.org/10.1103/" + apsjn + '.' + vol + '.' + pp + "\">" + 'DOI' + "</a>" + ']</b>')
                else:
                    letnum = volregex.search(vol)
                    if letnum:
                        l = letnum.group('let')
                        n = letnum.group('num')
                        ref_out.append('  <b>[' + "<a href=\"http://crossref.org/guestquery/?title=" + str(jn) + l + "&amp;volume=" + n + "&amp;page=" + pp + "\">" + 'CrossRef' + "</a>" + ']</b>')
                    else:
                        ref_out.append('  <b>[' + "<a href=\"http://crossref.org/guestquery/?title=" + str(jn) + "&amp;volume=" + vol + "&amp;page=" + pp + "\">" + 'CrossRef' + "</a>" + ']</b>')
...
#remember, all this is only if it is not in INSPIRE, and finally if nothing else works, let Google try it. 
            ...
            if clean_report:
                if re.search("\d{4}\.\d{4}", clean_report) or re.search("\/\d{7}", clean_report):
                   eprint = True
                if (not(display_journal or eprint)):
                   ref_out.append(' ' + display_report)
                if display_journal and eprint:
                   ref_out.append(' ' + '[' + display_report + ']')
                elif eprint:
                   ref_out.append(' ' + display_report)
                if not display_journal:
                   ...
                   c_count = str(len(perform_request_search(p=search_arg)))
                   ref_out.append('  <b>[' + "<a href=\"/search?ln=en&amp;p=fin" + "+" + "c" + "+" + clean_report + "\">" + c_count + "</a>" + ' citations in HEP]</b>')
                if eprint: 
                   ref_out.append('  <b>[' + "<a href=\"http://arxiv.org/abs/" + clean_report + "\">" + 'arXiv' + "</a>" + ']</b>')
                elif not display_journal:
                   ref_out.append('  <b>[' + "<a href=\"http://www.google.com/&#35;hl=en&amp;output=search&amp;sclient=psy-ab&amp;q=%22" + clean_report + "%22\">" + 'Google' + "</a>" + ']</b>')
